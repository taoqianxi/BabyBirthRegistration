...
//			level: "level",			//层级
  ...
  DTree.prototype.setting = function(options) {
    ...
    this.scroll = this.options.scroll || OPTIONS.scroll || this.elem;
    ...
    if(this.accordion) {
      ...
      this.initLevel = this.options.initLevel || OPTIONS.initLevel || 2;
    }
    this.type = this.options.type || OPTIONS.type || "load";
    this.cache = (typeof (this.options.cache) === "boolean") ? this.options.cache : (typeof (OPTIONS.cache) === "boolean") ? OPTIONS.cache : true;
    this.record =  this.options.record || OPTIONS.record || false; //开启数据记录模式，默认false
    this.load = (typeof (this.options.load) === "boolean") ? this.options.load : (typeof (OPTIONS.load) === "boolean") ? OPTIONS.load : true;
    this.none = this.options.none || OPTIONS.nont || "无数据";
    this.tempHeight = this.options.height || OPTIONS.height;
    ...
    this.iconfont = this.options.iconfont || OPTIONS.iconfont || DTREEFONT;
    this.iconfontStyle = this.options.iconfontStyle  || OPTIONS.iconfontStyle || {};
    this.nodeIconArray = $.extend(nodeIconArray, this.options.nodeIconArray || OPTIONS.nodeIconArray) || nodeIconArray;
    this.leafIconArray = $.extend(leafIconArray, this.options.leafIconArray || OPTIONS.leafIconArray) || leafIconArray;
    this.skin = this.options.skin || OPTIONS.skin || "theme";
    if(this.skin == "layui"){
      this.line = (typeof (this.options.line) === "boolean") ? this.options.line : (typeof (OPTIONS.line) === "boolean") ? OPTIONS.line : true;
      this.ficon = this.options.ficon || OPTIONS.ficon || "7";
      this.fnodeIcon = (typeof this.ficon === 'string' || typeof this.ficon === 'number') ? (this.ficon == "-1" ? "-1" : "1") : this.ficon[0];
      this.fleafIcon = (typeof this.ficon === 'string' || typeof this.ficon === 'number') ? this.ficon : this.ficon[1];
      this.icon = this.options.icon || OPTIONS.icon || "-1";
      this.nodeIcon = (typeof this.icon === 'string' || typeof this.icon === 'number') ? (this.icon == "-1" ? "-1" : "-1") : this.icon[0];
      this.leafIcon = (typeof this.icon === 'string' || typeof this.icon === 'number') ? this.icon : this.icon[1];
    } else if(this.skin == "laySimple"){
      this.line = this.options.line || OPTIONS.line || false;
      this.ficon = this.options.ficon || OPTIONS.ficon || ["2","-1"];
      this.fnodeIcon = (typeof this.ficon === 'string' || typeof this.ficon === 'number') ? (this.ficon == "-1" ? "-1" : "2") : this.ficon[0];
      this.fleafIcon = (typeof this.ficon === 'string' || typeof this.ficon === 'number') ? this.ficon : this.ficon[1];
      this.icon = this.options.icon || OPTIONS.icon || "-1";
      this.nodeIcon = (typeof this.icon === 'string' || typeof this.icon === 'number') ? (this.icon == "-1" ? "-1" : "-1") : this.icon[0];
      this.leafIcon = (typeof this.icon === 'string' || typeof this.icon === 'number') ? this.icon : this.icon[1];
      ...
      this.line = this.options.line || OPTIONS.line || false;
      this.ficon = this.options.ficon || OPTIONS.ficon || "8";
      this.fnodeIcon = (typeof this.ficon === 'string' || typeof this.ficon === 'number') ? (this.ficon == "-1" ? "-1" : "1") : this.ficon[0];
      this.fleafIcon = (typeof this.ficon === 'string' || typeof this.ficon === 'number') ? this.ficon : this.ficon[1];
      this.icon = this.options.icon || OPTIONS.icon || "5";
      this.nodeIcon = (typeof this.icon === 'string' || typeof this.icon === 'number') ? (this.icon == "-1" ? "-1" : "0") : this.icon[0];
      this.leafIcon = (typeof this.icon === 'string' || typeof this.icon === 'number') ? this.icon : this.icon[1];
    }
    this.url = this.options.url || OPTIONS.url || "";
    this.async = (typeof (this.options.async) === "boolean") ? this.options.async : (typeof (OPTIONS.async) === "boolean") ? OPTIONS.async : true;
    this.asyncLoad = this.options.asyncLoad || OPTIONS.asyncLoad || [];
    this.headers = this.options.headers || OPTIONS.headers || {};
    this.method = this.options.method || OPTIONS.method || "post";
    this.dataType = this.options.dataType || OPTIONS.dataType || "json";
    this.contentType = this.options.contentType || OPTIONS.contentType || "application/x-www-form-urlencoded";
    this.defaultRequest = $.extend(this.defaultRequest, this.options.defaultRequest || OPTIONS.defaultRequest) || this.defaultRequest;
    this.filterRequest = this.options.filterRequest || OPTIONS.filterRequest || [];
    this.request = this.options.request || OPTIONS.request || {};
    this.response = $.extend(this.response, this.options.response || OPTIONS.response) || this.response;
    this.data = this.options.data || OPTIONS.data || null;
    this.dataFormat = this.options.dataFormat || OPTIONS.dataFormat || "levelRelationship";
    this.dataStyle = this.options.dataStyle || OPTIONS.dataStyle || "defaultStyle";
    this.errDataShow = this.options.errDataShow || OPTIONS.errDataShow || false;
    this.withCredentials = this.options.withCredentials || OPTIONS.withCredentials || false;
    ...
    this.formatter = $.extend(this.formatter, this.options.formatter || OPTIONS.formatter) || this.formatter;
    ...
    this.checkbar = this.options.checkbar || OPTIONS.checkbar || false;
    this.checkbarLoad = this.options.checkbarLoad || OPTIONS.checkbarLoad || "node";
    this.checkbarType = this.options.checkbarType || OPTIONS.checkbarType || "all";
    this.checkbarData = this.options.checkbarData || OPTIONS.checkbarData || "choose";
    this.checkbarFun =  $.extend(this.checkbarFun, this.options.checkbarFun || OPTIONS.checkbarFun) || this.checkbarFun;
    this.menubar = this.options.menubar || OPTIONS.menubar || false;
    this.menubarTips = $.extend(this.menubarTips, this.options.menubarTips || OPTIONS.menubarTips) || this.menubarTips;
    this.menubarFun = $.extend(this.menubarFun, this.options.menubarFun || OPTIONS.menubarFun) || this.menubarFun;
    this.toolbar = this.options.toolbar || OPTIONS.toolbar || false;
    this.toolbarWay = this.options.toolbarWay || OPTIONS.toolbarWay || "contextmenu";
    this.toolbarStyle = $.extend(this.toolbarStyle, this.options.toolbarStyle || OPTIONS.toolbarStyle) || this.toolbarStyle;
    this.toolbarLoad = this.options.toolbarLoad || OPTIONS.toolbarLoad || "node";
    this.toolbarShow = this.options.toolbarShow || OPTIONS.toolbarShow || ["add","edit","delete"];
    this.toolbarBtn = this.options.toolbarBtn || OPTIONS.toolbarBtn || null;
    this.toolbarExt = this.options.toolbarExt || OPTIONS.toolbarExt || [];
    this.toolbarFun = $.extend(this.toolbarFun, this.options.toolbarFun || OPTIONS.toolbarFun) || this.toolbarFun;
    this.useIframe = this.options.useIframe || OPTIONS.useIframe || false;
    this.iframeElem = this.options.iframeElem || OPTIONS.iframeElem || "";
    this.iframeUrl = this.options.iframeUrl || OPTIONS.iframeUrl || "";
    this.iframeLoad = this.options.iframeLoad || OPTIONS.iframeLoad || "leaf";
    this.iframeDefaultRequest = $.extend(this.iframeDefaultRequest, this.options.iframeDefaultRequest || OPTIONS.iframeDefaultRequest) || this.iframeDefaultRequest;
    this.iframeRequest = $.extend(this.iframeRequest, this.options.iframeRequest) || $.extend(this.iframeRequest, OPTIONS.iframeRequest) || this.iframeRequest;
    this.iframeFun = $.extend(this.iframeFun, this.options.iframeFun) || $.extend(this.iframeFun, OPTIONS.iframeFun) || this.iframeFun;
  ...
  DTree.prototype.reloadSetting = function(options) {
    ...
    this.async = (typeof (this.options.async) === "boolean") ? this.options.async : this.async;
    ...
    this.defaultRequest = $.extend(this.defaultRequest, this.options.defaultRequest) || this.defaultRequest;
        ...
        if(flag){
          ...
          if(checkArr && checkArr.length > 0){
            ...
            for (var i = 0; i < checkArr.length; i++) {
              ...
              var checked = checkData.isChecked;
