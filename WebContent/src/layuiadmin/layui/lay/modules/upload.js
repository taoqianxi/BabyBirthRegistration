layui.define(['layer','api' ], function(exports){
    "use strict";
    ...
    Class.prototype.upload = function(files, type){
        ...
        var that = this
            ...
            ,ajaxSend = function(){
                ...
                var successful = 0, aborted = 0,items = files || that.files || that.chooseFiles || elemFile.files ,allDone = function(){ //多文件全部上传完毕的回调
                ...
                layui.each(items, function(index, file){
                    ...
                    console.log("上传文件时的额外参数：",options.data)
                    /**
                     * 引入oss 上传签名
                     */
                    var prefix = options.data.prefix ? options.data.prefix :'BOSS/OTHER/';
                    layui.admin.req({
                        url: layui.api.OssService.ossSignatrue,
                        data: {fileName: prefix+file.name},
                        success: function (data) {
                            ...
                            $.ajax({
                                ...
                                success: function(res){
                                    ...
                                    model.msg='上传成功'
                                    if(!res){
                                        model.uploadUrl = data.host+"/"+data.key
                                        model.data={src:model.uploadUrl}
                                    }
                                    successful++;
                                    done(index, model);
                                    allDone();
                                    console.log("文件上传地址：",model.uploadUrl)
            ...
            ,send = function(){
                ...
                //上传前的回调
                if(options.before){
                    var beforeResult = options.before(args)
                    if(beforeResult !=undefined && !beforeResult){
                        console.log("资源文件验证不通过")
                        return;
                    }
                }
                //IE兼容处理
